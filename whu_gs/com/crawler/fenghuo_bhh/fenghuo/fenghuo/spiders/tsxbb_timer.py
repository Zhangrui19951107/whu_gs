...
class tsxbb_Spider(CrawlSpider):
    ...
    def parse_page(self, response):
        ...
        url = response.url
        new = 6
        l_time = updatetool.getPubDate("www.tsxbb.gov.cn")   # 最后一次新闻时间
        while new > 1 and hasNext:
            new = 0
            r = requests.get(url)
            root = bs(r.text)
            ...
            for table in tables:
                ...
                if time > l_time:
                    new += 1
                    curl = root.findAll("a", attrs={"href": re.compile("^\./20")})
                    curl = curl.get("href")
                    curl = curl[1:]
                    source = response.url
